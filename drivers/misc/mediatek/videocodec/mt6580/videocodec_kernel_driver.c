        ...
        case VCODEC_LOCKHW:
        {
                ...
                rHWLock.eDriverType == VAL_DRIVER_TYPE_VP8_DEC)
            {
                while (bLockedHW == VAL_FALSE)
                {
                    ...
                    {
                        if (rHWLock.eDriverType == VAL_DRIVER_TYPE_H264_DEC)    //for H264 hybrid decoder
                        {
                            spin_lock_irqsave(&OalHWContextLock, ulFlags);
        ...
        case VCODEC_WAITISR:
        {
                ...
                val_isr.eDriverType == VAL_DRIVER_TYPE_VP8_DEC)
            {
                if(val_isr.eDriverType == VAL_DRIVER_TYPE_H264_DEC)
                {
                    ...
                    if (grVcodecHWLock.pvHandle == (VAL_VOID_T *)pmem_user_v2p_video((unsigned long)val_isr.pvHandle)) //normal case
                    {
                        bLockedHW = VAL_TRUE;
                    }
                    else    //current process can not use HW
                    {
                        ...
                        MFV_LOGE("[ERROR] VCODEC_WAITISR, grVcodecHWLock.pvHandle = 0x%lx, current handle = 0x%lx\n", (unsigned long)grVcodecHWLock.pvHandle, pmem_user_v2p_video((unsigned long)val_isr.pvHandle));
                    }
                    mutex_unlock(&HWLock);
                    ...
                    if (grVcodecHWLock.pvHandle == (VAL_VOID_T *)pmem_user_v2p_video((unsigned long)val_isr.pvHandle)) //normal case
                    {
                        ...
                        {
                            MFV_LOGE("[ERROR] VCODEC_WAITISR, ISR set HWLockEvent error\n");
                        }
                    }
                    mutex_unlock(&HWLock);
...
#ifdef CONFIG_HAS_EARLYSUSPEND
    //register_early_suspend(&vcodec_early_suspend_handler); // Temp fix to avoid link error.
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
    //unregister_early_suspend(&vcodec_early_suspend_handler); // Temp fix to avoid link error.
#endif
